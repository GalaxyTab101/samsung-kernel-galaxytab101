...
int usbsvn_reset_resume(struct usb_interface *intf)
{
	...
	svn->suspended = 0;
	err = usbsvn_start(svn);
	/* remove from here. only control in usbsvn_notifier_event */
		...
		unsigned long event, void *ptr)
{
	...
	struct device *dev = &svn->usbdev->dev;
	printk(KERN_INFO "[%d] %s, event=%d, svn->usbsvn_connected=%d, dev->power.runtime_status=%d\n", 
		__LINE__, __func__, event, svn->usbsvn_connected, dev->power.runtime_status);
