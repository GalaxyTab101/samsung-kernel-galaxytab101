 ...
 */
struct early_suspend tegra_enterprise_panel_early_suspender;

static void tegra_enterprise_panel_early_suspend(struct early_suspend *h)
{
	if (num_registered_fb > 0)
		fb_blank(registered_fb[0], FB_BLANK_POWERDOWN);
}

static void tegra_enterprise_panel_late_resume(struct early_suspend *h)
{
	if (num_registered_fb > 0)
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	tegra_enterprise_panel_early_suspender.suspend = tegra_enterprise_panel_early_suspend;
	tegra_enterprise_panel_early_suspender.resume = tegra_enterprise_panel_late_resume;
	tegra_enterprise_panel_early_suspender.level = EARLY_SUSPEND_LEVEL_DISABLE_FB;
	register_early_suspend(&tegra_enterprise_panel_early_suspender);
#endif
