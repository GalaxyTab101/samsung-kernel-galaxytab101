 ...
 *
 * Copyright (c) 2010 NVIDIA Corp.  All rights reserved.
 * Copyright (c) 2010 Wei Ni <wni@nvidia.com>
 *
 *
...
#define MAX_CHANNEL_MAPPING	0x08
...
static struct hda_verb nvhdmi_basic_init_tegra[] = {
					...
					struct hdmi_audio_infoframe *ai)
{
	...
	case 0:
	case 2:
		ai->CA = channel_allocations[0].ca_index;
		break;
	case 4:
		ai->CA = channel_allocations[3].ca_index;
		break;
	case 6:
		ai->CA = channel_allocations[6].ca_index;
		break;
	case 8:
		ai->CA = channel_allocations[8].ca_index;
		break;
	}
	return ai->CA;
}
				       ...
				       struct hdmi_audio_infoframe *ai)
{
	...
	if (hdmi_channel_mapping[ca][1] == 0) {
		...
		for (; i < MAX_CHANNEL_MAPPING; i++)
			hdmi_channel_mapping[ca][i] = 0xf | (i << 4);
	}

	for (i = 0; i < 8; i++) {
		err = snd_hda_codec_write(codec, nid, 0,
...
{
	snd_hda_sequence_write(codec, nvhdmi_basic_init_tegra);
	return 0;
}
...
static struct hda_pcm_stream nvhdmi_pcm_digital_playback_tegra = {
	.substreams = 1,
	.channels_min = 1,
	.channels_max = 8,
...
static int nvhdmi_build_pcms(struct hda_codec *codec)
{
	...
	info->stream[SNDRV_PCM_STREAM_PLAYBACK]
				= nvhdmi_pcm_digital_playback_tegra;
...
struct hda_codec_preset snd_hda_preset_nvhdmi[] = {
	{ .id = 0x10de0020, .name = "Tegra HDMI", .patch = patch_nvhdmi },
	{} /* terminator */
};
